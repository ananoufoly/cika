...
def compute_score(member_rows: pd.DataFrame, params: ScoreParams) -> dict:
    ...
    s_soc = params.w_rep * df["rep"].iloc[0] + params.w_cent * df["cent"].iloc[0] + params.w_endf * df["end_f"].iloc[0] + params.w_ends * df["end_s"].iloc[0]
...
def run_calibration(member_df: pd.DataFrame) -> Tuple[Dict[str, float], np.ndarray]:
    ...
    if y.nunique() < 2: return {f: 0.0 for f in features}, np.zeros(len(member_df))
    lr = LogisticRegression().fit(X, y)
    return dict(zip(features, lr.coef_[0])), lr.predict_proba(X)[:, 1]

@dataclass
class SimulationResult:
    member_df: pd.DataFrame; calibrated_weights: Dict[str, float]; rho_star: float
...
def generate_population(pop: PopulationParams, macro: MacroEnvironment, params: ScoreParams, seed: int = 42) -> SimulationResult:
    ...
    return SimulationResult(member_df, weights, _spearman(member_df['score'], 1.0 - pd_star))

# ---------------------------------------------------------------------------
# 5. STREAMLIT UI
# ---------------------------------------------------------------------------
SCHEMA = {
    "n_groups": ("pop", int, 20, "Groups simulated."), "group_size_min": ("pop", int, 6, "Min members."), "group_size_max": ("pop", int, 20, "Max members."),
    "rtype_bidding_prob": ("pop", float, 0.50, "Bidding prob."), "rules_prob": ("pop", float, 0.75, "Rules prob."), "p_ontime_mean": ("pop", float, 0.80, "Avg on-time."),
    "stress_level": ("macro", float, 0.0, "Stress."), "within_group_corr": ("macro", float, 0.20, "Correlation."),
    "a": ("score", float, 0.80, "Decay."), "c_otr": ("score", float, 0.85, "Sigmoid center."), "w_rep": ("score", float, 5.0, "Reputation weight."),
    "seed": ("global", int, 42, "Seed.")
}

UI_GROUPS = {
    "Population": ["n_groups", "group_size_min", "group_size_max", "rtype_bidding_prob", "rules_prob", "p_ontime_mean"],
    "Macro": ["stress_level", "within_group_corr"],
    "Score": ["a", "c_otr", "w_rep"],
    "Randomness": ["seed"]
}

st.set_page_config(page_title="ROSCA Score Sequence 2", layout="wide")
st.title("ROSCA Credit Score — Full Sequence Simulator")
...
if st.button("Run Simulation & ML Calibration"):
    pop = PopulationParams(n_groups=vals["n_groups"], p_ontime_mean=vals["p_ontime_mean"])
    macro = MacroEnvironment(stress_level=vals["stress_level"])
    params = ScoreParams(a=vals["a"], c_otr=vals["c_otr"], w_rep=vals["w_rep"])
    
    result = generate_population(pop, macro, params, seed=vals["seed"])
    df = result.member_df
    
    col1, col2, col3 = st.columns(3)
    col1.metric("Hard Defaults", int(df["is_defaulter"].sum()))
    col2.metric("Spearman ρ*", f"{result.rho_star:.3f}")
    col3.metric("Avg Score", f"{df['score'].mean():.1f}")
    
    st.subheader("Pillar Importance (ML Calibrated Weights)")
